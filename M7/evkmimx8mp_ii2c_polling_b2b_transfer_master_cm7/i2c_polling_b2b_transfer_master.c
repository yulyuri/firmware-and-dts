/*
 * Minimal change: keep your structure, switch transfers to MPU6050 on I2C3.
 * - No pin mux changes
 * - No instance/clock overrides (uses whatever EXAMPLE_I2C_MASTER_BASEADDR is)
 * - Just replaces the board-to-board xfers with WHO_AM_I + 14-byte burst
 */
...
/* Your existing example defs */
#define I2C_MASTER_SLAVE_ADDR_7BIT 0x68U     /* IMU addr (use 0x69 if AD0=1) */
...
int main(void)
{
    ...
    /* ---- REPLACED: talk to the IMU instead of 0x7E board-to-board ---- */
    ...
    xfer.subaddress     = MPU6050_REG_PWR1;           /* 0x6B */
    ...
    /* Read WHO_AM_I (expect 0x68) */
    uint8_t who = 0xFF;
    memset(&xfer, 0, sizeof(xfer));
    xfer.slaveAddress   = I2C_MASTER_SLAVE_ADDR_7BIT;
    xfer.direction      = kI2C_Read;
    xfer.subaddress     = MPU6050_REG_WHOAMI;         /* 0x75 */
    xfer.subaddressSize = 1;
    xfer.data           = &who;
    xfer.dataSize       = 1;
    xfer.flags          = kI2C_TransferDefaultFlag;
    ...
    /* Burst read 14 bytes from ACCEL_XOUT (0x3B): AX..AZ, TEMP, GX..GZ */
    uint8_t raw[14];
    memset(&xfer, 0, sizeof(xfer));
    xfer.slaveAddress   = I2C_MASTER_SLAVE_ADDR_7BIT;
    xfer.direction      = kI2C_Read;
    xfer.subaddress     = 0x3B;                        /* ACCEL_XOUT_H */
    xfer.subaddressSize = 1;
    xfer.data           = raw;
